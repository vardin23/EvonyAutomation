...
threshold = 0.95 ##0.90 for original file
if (len(sys.argv) == 4 ):
...
#print(threshold,'len(sys.argv) is :', len(sys.argv))
...
#cv2.waitKey(0)
cv2.imwrite("debImgLat.png", img_rgb)
if ( (len(loc[0]) == 0) or ( (loc[1][0] == 0) and (loc[0][0] == 0) )):
    exit()
#print(loc)
xxyyzz = random.randint(    0   ,   len(loc[0])-1 )
print(loc[1][xxyyzz],loc[0][xxyyzz],sep=',')