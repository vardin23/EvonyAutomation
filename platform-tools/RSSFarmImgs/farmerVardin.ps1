...
$host.ui.RawUI.WindowTitle = 'VardinFarmer'
 $ldDir = "F:\SteamLibrary\LDPlayer\LD4\"
 $ldstopDir = $ldDir+"dnconsole.exe quit --index 1"
$ldstartDir = $ldDir+ "dnconsole.exe launch --index 1"

function terminator([int]$minTime){
	if ($minTime -gt 900){
		$ldstopDir | Invoke-Expression
		write-host "Terminator Shootin"
...
function availableMarches([int]$nMarches=5){
	...
	magick convert ../tmp/nMarchesUsed$instx.png -crop 269x30+80+476 ../tmp/nMarchesUsedS6$instx.png
	...
	$marchesNTimes[5] = py ..\captioner.py ../tmp/nMarchesUsedS6$instx.png 1
	...
	if($availableMarches -eq 0){
		...
		$min = ([int]$min)+400
		write-host "Sleeping for " ($min - 400)
		Start-Sleep ($min-400)
		return $min
	}
...
while ($true){
	...
	$coord = (py locFinder.py ..\tmp\farmScreen.png stone_tile.jpg)
	while($avMar -ge 0 ){
		if($i % 17 -eq 0){..\adb -s $instx shell input tap 384 1054; $i+=1 ; continue}
		if( $i%25 -eq 0){$avMar = availableMarches;write-host -nonewline 'available marches ->'$avMar;$i+=1}
		if($avMar -ge 390){write-host 'slept for :'($avMar-400) -nonewline; $avMar = availableMarches ; continue}
		randMover 375 640 80
		screenSnap 'farmScreen.png'
		#[string]$corScr=(py locFinder.py ..\tmp\farmScreen.png .jpg)
		[string]$coord=(py locFinder.py ..\tmp\farmScreen.png stone_tile.jpg) # to get some debug output tryremoving '[int]' , which makes $coord reference to function	
		echo 'coords:'$coord
		$cX = [int]$coord.Split(',')[0]
		$cY = [int]$coord.Split(',')[1] + 10
		$i+=1
