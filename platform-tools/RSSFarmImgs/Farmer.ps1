...
$host.ui.RawUI.WindowTitle = 'LiegeFarmer'
...
while ($true){
	...
	$coord = (py locFinder.py ..\tmp\farmScreen.png lumb_tile.jpg)
	while($avMar -ge 0 ){
		...
		screenSnap 'farmScreen.png'
		#[string]$corScr=(py locFinder.py ..\tmp\farmScreen.png .jpg)
		[string]$coord=(py locFinder.py ..\tmp\farmScreen.png lumb_tile.jpg) # to get some debug output tryremoving '[int]' , which makes $coord reference to function	
		echo 'coords:'$coord
		$cX = [int]$coord.Split(',')[0]
		$cY = [int]$coord.Split(',')[1] + 10
		$i+=1
